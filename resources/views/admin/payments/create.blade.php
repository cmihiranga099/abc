...
@section('title', 'Create Payment - EventPro')

@section('content')
<section class="min-h-screen bg-slate-50 py-8">
    <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
            <h1 class="text-2xl font-semibold text-slate-900">Create Payment</h1>
            <p class="text-sm text-slate-500">Record a new payment.</p>
        </div>

        <form action="{{ route('admin.payments.store') }}" method="POST" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            @csrf
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <select name="booking_id" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" required>
                        ...
                        @foreach($bookings as $booking)
                            <option value="{{ $booking->id }}" @selected(old('booking_id') == $booking->id)>#{{ $booking->id }} - {{ $booking->event_name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="amount" value="{{ old('amount') }}" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" required>
                ...
                <div>
                    ...
                    <input type="text" name="transaction_id" value="{{ old('transaction_id') }}" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="payment_gateway" value="{{ old('payment_gateway', 'manual') }}" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">
            ...
            <div class="mt-6 flex gap-3">
                ...
                <button type="submit" class="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700">Create Payment</button>
            </div>
        </form>
    </div>
</section>
