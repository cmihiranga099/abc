...
<section class="min-h-screen bg-slate-50 py-8">
    <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
            ...
            <p class="text-sm text-slate-500">Update booking details.</p>
        </div>

        <form action="{{ route('admin.bookings.update', $booking) }}" method="POST" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="package_id" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" required>
                        @foreach($packages as $package)
                            <option value="{{ $package->id }}" @selected(old('package_id', $booking->package_id) == $package->id)>{{ $package->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="event_name" value="{{ old('event_name', $booking->event_name) }}" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" required>
                ...
                <div>
                    ...
                    <input type="date" name="event_date" value="{{ old('event_date', $booking->event_date->format('Y-m-d')) }}" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" required>
                ...
                <div>
                    ...
                    <input type="text" name="location" value="{{ old('location', $booking->location) }}" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" required>
                ...
                <div>
                    ...
                    <input type="number" name="guest_count" value="{{ old('guest_count', $booking->guest_count) }}" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" required>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="total_price" value="{{ old('total_price', $booking->total_price) }}" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" required>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="special_requirements" rows="3" class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">{{ old('special_requirements', $booking->special_requirements) }}</textarea>
                </div>
            </div>
